"""
QR -> VC -> CA30-shuffled S-box -> No-fixed-output testing

Behavior:
 - Same device-secret & QR/VC scaffolding as before.
 - For each CA-shuffled candidate S-box (grouped 6->4 mapping), compute:
     fixed_count = number of x in 0..63 such that S(x) == (x & 0xF)
 - Score = fixed_count (lower is better; ideal = 0).
...
# ---------- No-fixed-output Test ----------
def compute_no_fixed_points(grouped_sbox):
    """
    For grouped_sbox (6->4), define a 'fixed-output' when:
        flat[x] == (x & 0xF)
    Count such occurrences over x in 0..63.
    Return (fixed_count, list_of_fixed_inputs)
    """
...
def main():
    ...
    best_candidate = None
    best_score = None   # smaller fixed_count = better
    results = []

    print(f"[*] Starting {MAX_TRIES} CA30-shuffle attempts, measuring No-fixed-output...")
    ...
    for attempt in range(1, MAX_TRIES+1):
        ...
        fixed_count, fixed_list = compute_no_fixed_points(candidate)
        score = fixed_count
        ...
        results.append({
            ...
            "fixed_examples": fixed_list[:6]  # save up to 6 examples
        })

        if best_candidate is None or score < best_score:
            best_candidate = candidate
            best_score = score
            best_stats = {"fixed_count": fixed_count, "fixed_list": fixed_list}
            best_meta = {"attempt": attempt, "nonce": nonce, "fixed_count": fixed_count}

        if attempt % report_every == 0 or attempt == 1:
            print(f" attempt {attempt:3d}: fixed_count={fixed_count}  (nonce={nonce[:8]}...)")

        # early exit if perfect
        if score == 0:
            print(f"[*] Perfect candidate found at attempt {attempt} (no fixed outputs).")
            break

    # ---------- Print the chosen S-box ----------
    print("\n--- Best CA-shuffled S-box (grouped) by No-fixed-output score ---")
    ...
    # Print best fixed-output stats
    print("=== Best candidate No-fixed-output stats ===")
    print(f"Attempt: {best_meta['attempt']}, nonce: {best_meta['nonce'][:10]}..., fixed_count={best_meta['fixed_count']}")
    print("Example fixed inputs (if any):", best_stats["fixed_list"][:12])

    # Save summary and best
    with open("sbox_nofixed_summary.json", "w") as f:
        json.dump(results, f, indent=2)
    with open("best_sbox_by_nofixed.json", "w") as f:
        json.dump({"meta": best_meta, "sbox": best_candidate, "best_stats": best_stats}, f, indent=2)

    print("\n[*] Done. Best candidate saved to best_sbox_by_nofixed.json")
