...
# Configuration
# ----------------------------
DATASET_DIR = "./SboxScriptForTestingOnDataset/QR-images-Augmented/content/qr_augmented"
OUTPUT_DIR = "./SboxScriptForTestingOnDataset/optimized_augmented_results_with_iteration_pct"
ITERATIONS = 19
CA_STEPS = 50
CA_CELLS = 64
MAX_FILES = None
...
def process_file(path, out_dir, iterations=ITERATIONS):
    ...
    for i in range(iterations):
        ...
        # Replace the compression percentage calculation with:
        if len(compressed_bits) > 0:
            compression_pct = ((original_length - len(compressed_bits)) / len(compressed_bits)) * 100
        else:
            compression_pct = 100.0  # Or some other sentinel value
            print(f"  [!] Warning: Compressed to 0 bits at iteration {iter_no}")
        #compression_pct = ((original_length - len(compressed_bits)) / len(compressed_bits)) * 100
